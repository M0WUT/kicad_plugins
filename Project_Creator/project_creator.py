...
class ProjectHandler:
    ...
    def ensure_project_tracker_repo_exists(self):
        ...
        self.project_tracker_clone_path = get_temp_dir_path() / "project-tracker"
    ...
    def generate_next_project_number(self) -> int:
        ...
        if existing_projects:
            ...
            if set([int(x[0]) for x in existing_projects]) != set(
                [x for x in range(1, 1 + highest_project_number)]
            ):
                show_error(
                    f"Project numbering is not a continuous list for 1-{highest_project_number}. "
                    "Aborting.",
